...
StringRef getDispatchFnAttrName() { return "iree.dispatch_fn_name"; }

Optional<StringRef> getDispatchFnName(Operation *op) {
  FuncOp funcOp =
      (isa<FuncOp>(op) ? cast<FuncOp>(op) : op->getParentOfType<FuncOp>());
  if (!funcOp) return {};
  auto dispatchFnAttr =
      funcOp.getAttrOfType<StringAttr>(getDispatchFnAttrName());
  if (!dispatchFnAttr) return {};
  return dispatchFnAttr.getValue();
}

bool isDispatchFunctionImpl(FuncOp funcOp) {
  return !!(funcOp.getAttrOfType<StringAttr>(getDispatchFnAttrName()));
}
