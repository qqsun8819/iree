...
LogicalResult translateToVulkanSPIRVExecutable(
  ...
  for (auto executionModeOp :
    ...
    if (executionModeOp.execution_mode() == spirv::ExecutionMode::LocalSize) {
      ...
      for (auto valueAttr : executionModeOp.values()) {
        workGroupSize.push_back(valueAttr.cast<IntegerAttr>().getInt());
      }
      ...
      for (auto halEntryPointOp : halEntryPointOps) {
        if (executionModeOp.fn() == halEntryPointOp.sym_name()) {
          DenseIntElementsAttr currWorkgroupSizeAttr =
              halEntryPointOp.workgroup_size();
          halEntryPointOp.setAttr(
              "workgroup_size",
              DenseIntElementsAttr::get(currWorkgroupSizeAttr.getType(),
                                        workGroupSize));
        }
      }
    }
